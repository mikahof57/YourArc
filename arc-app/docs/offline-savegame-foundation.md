...
assignments/history, XP/level, economy ledger, inventory/equipment, wheel, missions,
...
`LocalEconomyService` provides atomic credit/debit, purchase, grant, equip/unequip, wheel,
module-reload, Mission/Achievement reward and verified-native-purchase operations. Purchases
debit, append the ledger and grant inventory in one save transaction. The wheel uses the live
1%/10%/40%/49% distribution and one claim per ARC day. It uses device cryptographic randomness
when available; an offline player can still tamper with local state, which is acceptable for
single-player but not proof suitable for competitive or real-money settlement.

The IAP function accepts only an event already verified by a future StoreKit/Google Play adapter;
it performs no receipt verification itself. External transaction IDs prevent duplicate grants.
No native billing button or provider integration is enabled. Stripe, checkout Edge Functions,
webhooks, `purchases`, `store_products` and their SQL remain untouched but are obsolete in the
target architecture and should be removed only in the later Supabase/IAP cutover.
...
Credits and ledger, Shop inventory/equip, wheel, Missions, Achievements, titles, private calendar,
